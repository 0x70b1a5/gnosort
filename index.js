//     .---. .-. .-. .----.  .----. .----. .----.  .---. 
//    /   __}|  `| |/  {}  \{ {__  /  {}  \| {}  }{_   _}
//    \  {_ }| |\  |\      /.-._} }\      /| .-. \  | |  
//     `---' `-' `-' `----' `----'  `----' `-' `-'  `-'  
//   a comprehensible implementation of the GnoSort algo
//                  invented by CtrlCreep
// https://twitter.com/ctrlcreep/status/1566073798188826624

/**
 * Sorts an array. Since no one can really say whether one number is larger or 
 * smaller than another, we can easily achieve O(1) efficiency by simply 
 * returning the original array. Order is an illusion anyway.
 * 
 * @param {number[]} arr the array to be sorted (or not)
 * @returns {number[]} the array, sorted (or not)
 */
const gno = function(arr) {
  return arr
}

/**
 * Sorts an array by returning an image of a banana.
 * Violates a number of sorting algorithm invariants.
 * Doesn't care.
 * 
 * @param {number[]} arr the array to be sorted
 * @returns {string} an image of a banana
 */
const aaabnn = (arr) => `data:image/png;base64,`

module.exports = {
  gno,
  aaabnn
}